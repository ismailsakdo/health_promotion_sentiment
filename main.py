        ...
        try:
            ...
            if len(df.columns) >= 3:
                ...
                if st.button("Download Sentiment Analysis Results"):
                    ...
                    st.markdown(f"[Download CSV](data:{'text/csv'};base64,{output_csv_file})", unsafe_allow_html=True)
                
                if st.button("Download Sentiment Distribution Chart"):
                    _, chart_png_file = save_files(df, fig, output_dir)
                    st.markdown(f"[Download PNG](data:{'image/png'};base64,{chart_png_file})", unsafe_allow_html=True)
            else:
